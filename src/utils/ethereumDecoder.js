...
export function decodeInputData(inputData, abiSignature) {
  try {
    ...
    if (functionSelector !== expectedSelector) {
      throw new Error(`Function selector mismatch. Expected: ${expectedSelector}, Got: ${functionSelector}`);
    }
    
    // Decode the parameters
    const decodedParams = iface.decodeFunctionData(abiSignature, `0x${inputData}`);
